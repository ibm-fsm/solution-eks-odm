...
**Option A: External Oracle Database (Production Recommended)**
Utilize a managed database service or self-managed EC2 Oracle database to completely offload persistence management and security compliance from the Kubernetes cluster.
...
For non-production pilot environments, the you may provide their own approved PostgreSQL container image to run within the same namespace.
...
helm push ibm-odm-prod-25.1.0.tgz oci://artifactory.gym.lan:8443/docker-local --insecure-skip-tls-verify

# 3. Pull the chart
helm pull ibm-helm/ibm-odm-prod --version 25.1.0
```
...
:::{.callout-tip}
### Lab & Sandbox Setup
If you are attempting to reproduce this environment in a local lab (e.g., k3s or Minikube) and need to manually install OPA Gatekeeper to simulate the constraint layer, please refer to the [Lab Setup Guide: OPA Gatekeeper Configuration](https://ibm-client-engineering.github.io/solution-journal-quantum-safe/src/guardium_insights/04_optional_services.html#configure-security-constraints-optional).
:::