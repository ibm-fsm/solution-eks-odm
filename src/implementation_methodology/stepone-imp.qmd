...
3.  **Create persistent value storage**
    ...
    ```
    Should output: ```bash persistentvolumeclaim/postgres-pvc created```
4.  **Create the deployment yaml**
    ...
    Expected output: ```bash deployment.apps/postgres created```
...
6. **Check that the volume is mounted**
    ...
    ```
    **Note:** if it gets stuck, run ```bash kubectl -n postgres describe pod postgres-xxxxx```
7. **Create the service file (once pod is running)**
    ...
    Expected output ```bash service/postgres created```
...
8. **Verify that the volume is running**
  ...
  You should now have postgres-pvc.yaml, postgres-deployment.yaml, and postgres-service.yaml under the /home/clouduser/k3s/postgres directory
